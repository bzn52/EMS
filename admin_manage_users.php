                  ...
                  <td>
                    <div style="display: flex; gap: 0.5rem; justify-content: center; flex-wrap: wrap;">
                      <?php if (!$isCurrentUser): ?>
                        ...
                        <button class="btn btn-sm" onclick="openRoleModal(<?= (int)$user['id'] ?>, '<?= e($user['name']) ?>', '<?= e($user['role']) ?>')">
                          <i class="fas fa-exchange-alt"></i> Change Role
                        </button>
                        
                        <button class="btn btn-sm btn-secondary" onclick="openPasswordModal(<?= (int)$user['id'] ?>, '<?= e($user['name']) ?>', '<?= e($user['role']) ?>')">
                          <i class="fas fa-key"></i> Reset Password
                        </button>
                        
                        <form method="post" style="display: inline;" onsubmit="return confirm('Delete user <?= e($user['name']) ?>?')">
                          <?= CSRF::field() ?>
                          <input type="hidden" name="user_id" value="<?= (int)$user['id'] ?>">
                          <input type="hidden" name="user_type" value="<?= e($user['role']) ?>">
